...
public class ImageGrinderTask extends DefaultTask {
	...
	@Nested
	ImageGrinderTaskApi api;

	public ImageGrinderTaskApi getApi() {
		return api;
	}

	public void setApi(ImageGrinderTaskApi api) {
		this.api = api;
	}

	@TaskAction
	public void performAction(InputChanges inputChanges) throws Exception {
		Objects.requireNonNull(api.grinder, "grinder");

		File cache = new File(getProject().getBuildDir(), "cache" + getName());
		if (!inputChanges.isIncremental()) {
			getProject().delete(api.getDstDir().getAsFile().get());
			map = HashMultimap.create();
		} else {
			readFromCache(cache);
		}
		WorkQueue queue = workerExecutor.noIsolation();
		for (FileChange fileChange : inputChanges.getFileChanges(api.getSrcDir())) {
	...
	public static abstract class RenderSvg implements WorkAction<RenderSvgParams> {
		...
		public void execute() {
			...
			Subpath subpath = Subpath.from(task.api.getSrcDir().getAsFile().get(), sourceFile);
			Img<?> img;
